...
class PhotosView: UIView,CAAnimationDelegate {
    ...
    func handleOrientationChanges(orientation:String)  {
    ...
    func pauseAllAnimations() {
        for aSubview in self.subviews {
            if let aContainerView = aSubview as? ContainerView {
                //pause all container view
                let pausedTime:CFTimeInterval = aContainerView.layer.convertTime(CACurrentMediaTime(), from: nil)
                aContainerView.layer.speed = 0.0
                aContainerView.layer.timeOffset = pausedTime
            }
        }
    }
    ...
    func resumeAnimations() {
        for aSubview in self.subviews {
            if let aContainerView = aSubview as? ContainerView {
                ...
                //set the new toValue for animation based on orientation
                let toY = self.frame.size.height + aContainerView.bounds.height
                let fromPoint = aContainerView.layer.position
                let toPoint:CGPoint = CGPoint(x: aContainerView.layer.position.x, y: toY)
                
                let movement = CABasicAnimation(keyPath: "position")
                movement.fromValue = NSValue(cgPoint: fromPoint)
                movement.toValue = NSValue(cgPoint: toPoint)
                movement.duration = aContainerView.duration
                movement.delegate = self
                
                //store a key value pair reference to imageContainerView
                movement.setValue(aContainerView, forKey: "containerView")
                
                aContainerView.layer.position = toPoint
                aContainerView.layer.fillMode = kCAFillModeForwards
                aContainerView.layer.add(movement, forKey: "move")
                
                //resume the animation
                let pausedTime:TimeInterval = aContainerView.layer.timeOffset
                aContainerView.layer.speed = 1.0
                aContainerView.layer.timeOffset = 0.0
                aContainerView.layer.beginTime = 0.0
                let timeSincePause:CFTimeInterval = aContainerView.layer.convertTime(CACurrentMediaTime(), from: nil) - pausedTime
                aContainerView.layer.beginTime = timeSincePause
            }
        }
    }
}
